...
NOM_JOUEUR_1 = "Valentin"
PION_JOUEUR_1 = "X"
NOM_JOUEUR_2 = "Guillaume"
PION_JOUEUR_2 = "O"
...
class Jeu(object):
    ...
    def nbre_cases_alignees_pion(self, pion):
        ...
        while True:
            ...
            if colonne < self._grille.nbre_colonnes - 1:
                ligne -= 1
                colonne += 1
            else:
                colonne_depart += 1
...
class JoueurHumain(object):
    ...
    def choix(self):
        ...
        :return: numéro de colonne (int) ou -1 (int)
        """
        while True:
...
class PartieConsole(object):
    def __init__(self, jeu, joueur1, joueur2):
        self._jeu = jeu
        self._joueur1 = joueur1
        self._joueur2 = joueur2
...
class IAMinMax(object):
    ...
    """
    def __init__(self, jeu, nom, pion, pion_adversaire, profondeur=1):
        """
    ...
    def choix2(self):
        ...
        for colonne in self._jeu.colonnes_jouables:
            ...
            # (la condition suivante sera toujours vérifiée au moins une fois car il y a au moins un coup jouable pour l'IA)
            if score > score_max:
                score_max = score
                coup_retenu = coup

            # ...puis annuler le coup de l'IA...
            self._jeu.annuler(coup)
            # ...et rechercher le score obtenu avec le premier coup possible suivant s'il existe
        ...
        print(self._nbre_coups_simules)
        self._nbre_coups_simules = 0
    ...
    def choix(self):
        ...
        # If faut retenir le coup qui donnera le score maximal pour l'IA.
        # On initialise donc "score_max" à la valeur la moins élevée...
        score_max = -self._score_max-1 # -1 pour être certain que "score > score_max" sera vérifié (sinon plantage)
        ...
        for colonne in self._jeu.colonnes_jouables:
            ...
            # ...puis on simule le coup suivant de l'adversaire
            # et on récupère le score obtenu en évaluant la situation du point de vue de l'IA
            score = self.simuler_coup_adversaire_ab(self._profondeur - 1, score_max, self._score_max)
    ...
    def simuler_coup_adversaire_ab(self, profondeur, alpha, beta):
        """
        Simulation des coups possibles pour l'adversaire
        If faut retenir le score minimal du point de vue de l'IA

        :param profondeur: profondeur maximale de recherche des coups
        :param alpha: valeur minimale que le joueur peut espérer obtenir pour le coup à jouer
        :param beta: valeur maximale que le joueur peut espérer obtenir pour le coup à jouer
        :return: score obtenu le plus défavorable du point de vue de l'IA
        """
        # Si la profondeur maximale est atteinte (-1 si départ avec profondeur=0, 0 autrement) ou si le jeu est fini,
        # il faut évaluer la situation suite au dernier coup joué. Ce coup a été joué par l'IA
        ...
        for colonne in self._jeu.colonnes_jouables:
            self._nbre_coups_simules += 1
            coup = self._jeu.jouer(colonne, self._pion_adversaire)
            score = self.simuler_coup_ia_ab(profondeur - 1, alpha, beta)
            self._jeu.annuler(coup)
    ...
    def simuler_coup_ia_ab(self, profondeur, alpha, beta):
        """
        Simulation des coups possibles pour l'IA
        If faut retenir le score maximal du point de vue de l'IA
        ...
        for colonne in self._jeu.colonnes_jouables:
            self._nbre_coups_simules += 1
            coup = self._jeu.jouer(colonne, self.pion)
            score = self.simuler_coup_adversaire_ab(profondeur - 1, alpha, beta)
            self._jeu.annuler(coup)
...
j1 = JoueurHumain(puissance4, NOM_JOUEUR_1, PION_JOUEUR_1)
j2 = JoueurHumain(puissance4, NOM_JOUEUR_2, PION_JOUEUR_2)
ia_aleatoire = IaAleatoire(puissance4, 'Aleatoire', PION_JOUEUR_2)
ia_minmax1 = IAMinMax(puissance4, "Minmax1", "X", "O", PROFONDEUR_IA)
ia_minmax2 = IAMinMax(puissance4, "Minmax2", "O", "X", PROFONDEUR_IA)

partie = PartieConsole(puissance4, ia_minmax1, ia_minmax2)
partie.jouer()
